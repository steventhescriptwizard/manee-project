...
<div class="max-w-2xl mx-auto">
    <div class="mb-8 flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Tax</h1>
            <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Update tax details.</p>
        </div>
    </div>

    <form action="{{ route('admin.taxes.update', $tax) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-slate-200 dark:border-gray-700 p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select 
                    ...
                    class="w-full rounded-lg border-slate-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:border-blue-500 focus:ring-blue-500 sm:text-sm"
                >
                    <option value="sales" {{ old('type', $tax->type) == 'sales' ? 'selected' : '' }}>Sales Tax</option>
                    <option value="purchase" {{ old('type', $tax->type) == 'purchase' ? 'selected' : '' }}>Purchase Tax</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
