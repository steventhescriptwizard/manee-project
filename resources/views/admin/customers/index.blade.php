...
@section('title', 'Manajemen Pelanggan - Maneé Admin')
...
<div class="flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Pelanggan</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Kelola data pelanggan dan pantau aktivitas belanja mereka.</p>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.customers.create') }}" class="flex items-center gap-2 bg-slate-900 dark:bg-blue-600 text-white px-6 py-3 rounded-xl font-bold text-sm hover:opacity-90 transition-all shadow-lg shadow-slate-200 dark:shadow-none">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-900 p-6 rounded-2xl border border-slate-200 dark:border-gray-800 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Total Pelanggan</p>
                    <p class="text-2xl font-black text-slate-900 dark:text-white">{{ $customers->total() }}</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-800">
                        ...
                        <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Info Kontak</th>
                        <th class="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400 text-center">Pesanan</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-slate-50/30 dark:hover:bg-gray-800/20 transition-all group">
                        ...
                        <td class="px-8 py-5 whitespace-nowrap text-center">
                            <div class="inline-flex items-center gap-2 px-3 py-1 bg-slate-100 dark:bg-gray-800 rounded-full">
                                <span class="material-symbols-outlined text-[14px] text-blue-600">shopping_bag</span>
                                <span class="text-xs font-black text-slate-900 dark:text-white">{{ $user->orders_count ?? 0 }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-5 whitespace-nowrap text-center">
                        ...
                        <td class="px-8 py-5 whitespace-nowrap">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('admin.customers.show', $user) }}" class="inline-flex size-10 items-center justify-center rounded-xl bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm group/btn" title="Lihat Profil">
                                    <span class="material-symbols-outlined text-[20px]">visibility</span>
                                </a>
                                <a href="{{ route('admin.customers.edit', $user) }}" class="inline-flex size-10 items-center justify-center rounded-xl bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm" title="Edit">
                                    <span class="material-symbols-outlined text-[20px]">edit</span>
                                </a>
                                <form action="{{ route('admin.customers.destroy', $user) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pelanggan ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-24 text-center">
                            <div class="flex flex-col items-center gap-4 opacity-30">
                                ...
                                <p class="text-slate-900 font-black uppercase tracking-widest">Belum ada pelanggan ditemukan</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
