...
@php
    $breadcrumbs = [
        ...
        ['label' => 'Edit Variant', 'url' => null]
    ];
@endphp

@section('title', 'Edit Variant - Maneé Admin')
...
<div class="max-w-2xl mx-auto flex flex-col gap-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Variant</h1>
    </div>

    <form action="{{ route('admin.products.variants.update', [$product, $variant]) }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-xl border border-slate-200 dark:border-gray-800 shadow-sm space-y-4">
            ...
            <div x-data="{ photoName: null, photoPreview: null }">
                ...
                <div class="mt-2 flex items-center gap-4">
                    <div x-show="!photoPreview" class="h-24 w-24 rounded-lg bg-slate-100 dark:bg-gray-800 flex items-center justify-center border border-slate-200 dark:border-gray-700 overflow-hidden">
                        @if($variant->image_path)
                            <img src="{{ asset('storage/' . $variant->image_path) }}" class="w-full h-full object-cover">
                        @else
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="color" value="{{ old('color', $variant->color) }}" placeholder="e.g. Red, #FF0000" class="w-full rounded-lg border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-slate-900 dark:text-white focus:ring-blue-600 focus:border-blue-600">
            ...
            <div>
                ...
                <input type="text" name="variant_name" value="{{ old('variant_name', $variant->attributes['name'] ?? '') }}" placeholder="Leave blank to auto-generate" class="w-full rounded-lg border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-slate-900 dark:text-white focus:ring-blue-600 focus:border-blue-600">
            ...
            <div class="pt-4 border-t border-slate-200 dark:border-gray-800">
                <div class="flex items-center gap-2 mb-4">
                    <input type="checkbox" id="track_inventory" name="track_inventory" value="1" {{ old('track_inventory', $variant->track_inventory) ? 'checked' : '' }} class="rounded border-slate-300 text-blue-600 focus:ring-blue-600">
                ...
                <div id="stock_field">
                    ...
                    <input type="number" name="current_stock" value="{{ old('current_stock', $variant->stocks->sum('current_stock')) }}" class="w-full rounded-lg border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-slate-900 dark:text-white focus:ring-blue-600 focus:border-blue-600">
             ...
             <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="weight" value="{{ old('weight', $variant->weight) }}" class="w-full rounded-lg border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-slate-900 dark:text-white focus:ring-blue-600 focus:border-blue-600">
                </div>
            </div>
        </div>
